...
class simclr(nn.Module):
  ...
  def momentum(self, x_start: float, z: torch.Tensor, step: float = 0.05, discount: float = 0.7): 
  ...
  def loss_cal(self, x, x_aug):
    self.tau1 = self.momentum(self.tau1, x)
    batch_size, _ = x.size()    # x.shape: [128,96]
    print("x.size(): ", x.size(), x.shape)
    
    x_abs = x.norm(dim=1)
    x_aug_abs = x_aug.norm(dim=1)
    # sim_matrix = torch.exp(sim_matrix / 0.2)
    sim_matrix = torch.einsum('ik,jk->ij', x, x_aug) / torch.einsum('i,j->ij', x_abs, x_aug_abs)
    sim_matrix = torch.exp(sim_matrix / self.tau1)
    print("sim_matrix shape: ", sim_matrix.shape)
    pos_sim = sim_matrix[range(batch_size), range(batch_size)]
    print("pos_sim shape: ", pos_sim.shape)
    sys.exit()
    # loss = pos_sim / (sim_matrix.sum(dim=1) - pos_sim)    # GraphCL
    loss = pos_sim / (sim_matrix.sum(dim=1))                # GRACE or GLATE
...
if __name__ == '__main__':
    ...
    for epoch in range(1, epochs+1):
        ...
        for data in dataloader:
            ...
            data = data.to(device)
            print("data.batch, data.num_graphs", data.batch, data.num_graphs, data.batch.shape)
    ...
    with open('./logs/log_' + args.DS + '_' + args.aug, 'w') as f:
